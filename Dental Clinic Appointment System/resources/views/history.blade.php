...
<body>
    ...
    function calculateTotalRevenue() {
        let total = 0; // Initialize total revenue variable

        // Select all rows in the table body
        document.querySelectorAll('.history-table tbody tr').forEach(row => {
            const revenueCell = row.cells[6]; // Revenue is in the 7th column (zero-based index)
            
            if (revenueCell) {
                // Extract revenue value, remove '₱', and convert to float
                const amount = parseFloat(revenueCell.innerText.replace('₱', '').replace(',', '').trim());
                
                if (!isNaN(amount)) {
                    total += amount; // Add valid revenue to the total
                }
            }
        });

        // Update the Total Revenue in the DOM
